...
export default function TwoEditableSelectsForm() {
    const methods = useForm();
    const previousFormat = useRef({});
    const previousQuantity = useRef({});
    ...
    const printSize = {
        ...
        onChange: (val, helpers) => {
            if (helpers?.setCustomDefaults && val?.width && val?.height) {
                previousFormat.current = val;
            }
        },
        getPreviousDefaults: () => previousFormat.current,
    };

    const copiesPerDesign = {
        label: 'Тираж',
        fields: ['copiesPerDesign'],
        fieldLabels: { quantity: 'Тираж, шт' },
        fieldValidation: { quantity: { min: 1, max: 100000 } },
        layout: 'inline',
        defaultValue: '50',
        groupedOptions: [
            {
                group: 'Стандартные тиражи',
                options: [
        ...
        onChange: (val, helpers) => {
            if (helpers?.setCustomDefaults && val?.quantity) {
                previousQuantity.current = val;
            }
        },
        getPreviousDefaults: () => previousQuantity.current,
    };
    const printColorMode = {label: "Односторонняя / двусторонняя печать"}
    const printMaterial = {label: "Материал печати"}

    // Добавить материалы для печати:
    /*
    - office80: {label: "Бумага офисная, 80г/м²", default: true,}
    - maxiSilk130: {label: "Бумага мелованная MaxiSilk, 130г/м²",}
    - maxiSilk170: {label: "Бумага мелованная MaxiSilk, 170г/м²",}
    - maxiSilk300: {label: "Бумага мелованная MaxiSilk, 300г/м²",}
    - lenWhite280: {label: "Бумага тисненая «Лён (белый)», 280г/м²",}
    - lenYellow280: {label: "Бумага тисненая «Лён (Слоновая кость)», 280г/м²",}
    - majestic: {label: "Бумага Majestic",}
    - touchCover: {label: "Бумага TouchCover",}
    - laminationFront: {label: "Ламинация 1 стороны"}
    - laminationBack: {label: "Ламинация 2 стороны"}
    - stickerPaper: {label: "Бумага самоклеящаяся"} //Если выбран этот пункт, устанавливать `laminationFront` равным true, а `laminationBack` делать неактивным

    - roundedCorners: {label: "Скругление углов"}
    - holeDrilling: {label: "Сверление отверстий"}
    - creasingLines: {label: "Биговка (линии сгиба)"}
    - paperFolding: {label: "Фальцовка"}
    - perforationLines: {label: "Перфорация"}
    - bindingType: {label: "Брошюровка (тип переплёта)"}
    - documentNumbering: {label: "Нумерация документов"}
    - sheetCollation:  {label: "Листоподбор"}
*/
        ...
        <Container maxWidth="md">
            ...
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit(onSubmit)}>
                    <Box sx={{ mb: 4 }}>
                        <EditableSelect name="printSize" config={printSize} />
                    </Box>

                    <Box sx={{ mb: 4 }}>
                        <EditableSelect name="copiesPerDesign" config={copiesPerDesign} />
                    </Box>
